...
class Common(object):
    ...
    def __init__(self):
        ...
        self.GAE_AUTOSWICH        = self.CONFIG.getint('gae', 'autoswtich') if self.CONFIG.has_option('gae', 'autoswtich') else 1
...
def gaeproxy_handler(sock, address, hls={'setuplock':gevent.coros.Semaphore()}):
    ...
    if method == 'CONNECT':
        ...
        if host.endswith(common.GOOGLE_SITES) and host not in common.GOOGLE_WITHGAE:
            logging.info('%s:%s HOST "%s %s:%d HTTP/1.1" - -' % (remote_addr, remote_port, method, host, port))
    ...
    if need_direct:
        ...
        try:
            ...
            if response.status in (400, 405):
                common.GAE_CRLF = 0
            logging.info('%s:%s HOST "%s %s HTTP/1.1" %s %s' % (remote_addr, remote_port, method, path, response.status, response.msg.get('Content-Length', '-')))
...
def socks5proxy_handler(sock, address, hls={'setuplock':gevent.coros.Semaphore()}):
    ...
    remote_addr, remote_port = address
    logging.info('%s:%s SOCK5 "POST %s SOCKS/5" - -' % (remote_addr, remote_port, common.SOCKS5_FETCHSERVER))
